# 概要
- 本家Gemini PWA( https://github.com/ona-oni/geminipwa )に感銘を受けた他の人が触発され、他の方の成果も取り入れつつ、自分用にいじった派生コードを公開しているリポジトリ
...
*  （⚠️の数は重要度を指します）
*   ⚠️⚠️tomo00024様が、より洗練された形式のアプリケーション（https://github.com/tomo00024/geminipwa-NV ）を公開されました。今後こちらも追随し、随時こちらのリポジトリにも導入させていただく予定です。2025/09/10
*   他の方のコードから更に良いところを学んでいきたいと思料しておりますが、大規模な変更はデバッグ環境が十全に整うのを待ったのちに作業する方が安全だと判断しました。2025/08/25、2025/08/30追記
*   kinkan04氏のPWA（https://github.com/kinkan04/Gemini-PWA-Mk-II ）の校正機能が好評のため、こちらのリポジトリのコードにも（仕様をこちらに適合させつつ）移植・導入させていただきました。校正のほかにも自動的にテキストを差し替える用途などに役立つ大変有用な機能と存じます。有効化する場合、“その他設定→アプリの挙動を調整→上級ユーザー向けオプション→校正機能を使用する”をオンにし、セットアップしてください。2025/08/10
*   ⚠️試製版“Twin-engine”も実装されていますが、調整中のため使用しないでください。現段階においては、“Twin-engine設定を表示する”を有効化しないでください。
（使用方法を誤ると大量にトークンを浪費する事態になり得ます）2025/07/19
*   ⚠️⚠️⚠️⚠️⚠️過去のセッション履歴やプロンプト等の重要なデータは、各ユーザー様の任意のタイミングでバックアップしていただきますようお願いいたします。 
*   各ボタン類の表示/非表示は設定画面から選択可能です。
*   ⚠️APIプロバイダーのオプションにLLM Aggregatorを追加しました。OpenRouter（https://openrouter.ai/api/v1 ）やHugging Face（https://router.huggingface.co/v1) ）などのAPIを使用して任意のモデルを利用することができます。どのアグリゲーターを用いるかはマニュアルでURLを指定します。利用を始める際は、まずは無料のモデルを用いてテストを行なってください。（デフォルトでオプションとして備えられているOpenRouter用モデルは無料使用可能です）
...
-  履歴画面の視認性向上、履歴リストの背景に、最新の会話内容をうっすら表示する機能をオプション搭載。
-  動作の最適化（イベント処理の軽量化・DOM描画の最適・DB初期化処理の安定化。バグが出たらすみません）
-  ClaudeとGeminiの各種モデル追加
### 【2025/12/17】
-  思考プロセスの操作性向上
思考プロセスの出力欄の下部余白（背景部分）をクリック/タップすることで、アコーディオンを開閉できるようにしました。（テキスト選択時は閉じないように制御）
-  画面遷移時の編集状態保持
メッセージ編集中に「設定」や「履歴」画面へ移動しても、編集モードが勝手に解除されないように修正しました。
### 【2025/12/08】
-  ⚠️DeepSeek APIのデフォルトの接続先を、中継サーバーを介する方式に変更しました
-  （DeepSeek APIのCORSポリシーにより、ブラウザから直接APIを呼び出すことができなくなりました。
-  このため、Cloudflare Workersを用いたCORS回避用プロキシを設置し、そのURLをデフォルトの接続先としました。）
### 【2025/12/06】
-  履歴パネル・チャット画面・設定パネルそれぞれのフォント倍率を 0.5〜2.0倍で調整できるスライダーを追加。
-  送信中の入力を自由化: メッセージの送信中やAIの応答中でもフッター（入力欄、添付ボタン、API設定など）をロックせず、次の入力や設定変更を行えるようにしました。
-  PC操作の改善: 画面左右の余白部分でも「マウスホイールでのスクロール」および「ホイール押し込みによるオートスクロール」ができるように実装しました。
-  スマホでtextarea / 入力欄上では画面切り替えスワイプが発動しないように変更。
-  応答時間表示: AI応答待ち中に経過秒数（例: 1.5s）を表示する機能を追加。
-  取り扱い注意領域にセキュリティ設定を追加し、APIキーの黒塗りを解除して表示する機能を追加。
-  またセキュリティ設定内に、LLM Aggregatorで任意のドメインをバックエンドに指定可能にする自由化設定を追加。
-  「画面が裏に行っている間は、応答待ち時間を0秒にする」ように変更
-  アプリ起動時にブラウザへストレージの自動削除防止（永続化）を自動リクエストし、データが勝手に消えにくくなるよう保護を強化
-  ヘッダー/フッターのタップで最上部/最下部へ移動する設定を追加。設定項目の余白を詰めて一覧性を高めるオプションを追加。
-  また、設定項目のバブル表示（枠線・背景）を無効にするオプションを追加。
### 【2025/11/24】
-  モデルオプションにClaude Opus 4.5を追加
### 【2025/11/21】
-  モデルオプションにGrok4.1系を追加
### 【2025/11/19】
